...
@section('content')
  <div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-10">
      <h2>Thùng rác - Đơn hàng</h2>
      <ol class="breadcrumb">
        <li><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
        <li><a href="{{ route('admin.order.index') }}">Đơn hàng</a></li>
        <li class="active"><strong>Thùng rác</strong></li>
      </ol>
    </div>
    <div class="col-lg-2">
        ...
        <div class="ibox float-e-margins">
          <div class="ibox-title">
            ...
            <div class="ibox-tools">
              <span class="badge bg-danger">{{ $orders->total() }} đơn hàng đã xóa</span>
            </div>
          </div>
          <div class="ibox-content">
            ...
            @if($orders->count() == 0)
              <div class="text-center py-5">
                ...
                <p class="text-muted">Không có đơn hàng nào đã bị xóa</p>
                <a href="{{ route('admin.order.index') }}" class="btn btn-primary">
                ...
                <table class="table table-bordered table-hover align-middle" style="border-radius: 12px; overflow: hidden;">
                  <thead class="thead-light" style="background: #f8f9fa;">
                    <tr>
                      ...
                      <th class="text-center" style="width:100px">Ngày xóa</th>
                  ...
                  <tbody>
                    @foreach($orders as $order)
                      <tr id="order-{{ $order->id }}">
                        ...
                        <td class="text-center">
                          <div>{{ $order->deleted_at->format('d/m/Y') }}</div>
                          <small class="text-muted">{{ $order->deleted_at->format('H:i') }}</small>
                        </td>
                        <td class="text-center">
                          <div class="btn-group" role="group">
                            ...
                            <form action="{{ route('admin.order.restore', $order->id) }}" method="POST" style="display: inline;">
                              @csrf
                              <button type="submit" class="btn btn-xs btn-success" title="Khôi phục" onclick="return confirm('Bạn có chắc muốn khôi phục đơn hàng này?')">
                                <i class="fa fa-undo"></i>
                              </button>
                            </form>
                            <form action="{{ route('admin.order.force-delete', $order->id) }}" method="POST" style="display: inline;">
                              @csrf
                              @method('DELETE')
                              <button type="submit" class="btn btn-xs btn-danger" title="Xóa vĩnh viễn" onclick="return confirm('Bạn có chắc muốn xóa vĩnh viễn đơn hàng này? Hành động này không thể hoàn tác!')">
